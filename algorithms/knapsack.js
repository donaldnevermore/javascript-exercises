function maxValue(weights, values, index, weightAvailable) {
    ...
    if (weights[index] > weightAvailable) {
        ...
        const withIndex = values[index] + maxValue(weights, values, index - 1,
...
function fastMaxValue(weights, values, index, weightAvailable, memory) {
    ...
    if (weights[index] > weightAvailable) {
        ...
        const withIndex = values[index] + fastMaxValue(weights, values, index - 1,
            weightAvailable - weights[index], memory);
...
const result = fastMaxValue(weights, values, weights.length - 1, weightAvailable, memo);
console.log(result);

weights2 = [5, 3, 2];
values2 = [9, 7, 8];
const result2 = maxValue(weights2, values2, weights2.length - 1, weightAvailable);
console.log(result2);
